...
class Tester:
    ...
    def setup(self,**kwargs):
        ...
        if self.remoteA != 'A':
            call([exe,'purge'] + \
                  ...
                  [self.remoteA],env=self.config.rclone_env) 
        if self.remoteB != 'B':
            call([exe,'purge'] + \
    ...
    def sync(self,flags=None,configpath='config.py'):
        ...
        if self.remoteA != 'A':
            call([exe,'sync'] + \
                  self.config.rclone_flags + self.config.rclone_flagsA + \
                  ['A/',self.remoteA],env=self.config.rclone_env) 
        if self.remoteB != 'B':
            call([exe,'sync'] + \
                  self.config.rclone_flags + self.config.rclone_flagsB + \
                  ['B/',self.remoteB],env=self.config.rclone_env)   
        ...
        if self.remoteA != 'A':
            call([exe,'sync'] + \
                  self.config.rclone_flags + self.config.rclone_flagsA + \
                  [self.remoteA,'A/'],env=self.config.rclone_env) 
        if self.remoteB != 'B':
            call([exe,'sync'] + \
                  self.config.rclone_flags + self.config.rclone_flagsB + \
...
def tree(path):
    ...
    for dirpath, dirnames, filenames in os.walk(path,followlinks=False):
        ...
        exc = {'.DS_Store'}
        files.extend(os.path.join(dirpath,filename) for filename in filenames if filename not in exc)
