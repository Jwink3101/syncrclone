...
@pytest.mark.parametrize(
    ("remoteA,renamesA,workdirA," "remoteB,renamesB,workdirB," "compare"), MAIN_TESTS
)
def test_main(
    remoteA,
    renamesA,
    ...
    test.config.rclone_flags = [
        "--fast-list"
    ]  # Will be ignored if not supported but good to have otherwise

    test.config.filter_flags = ["--filter", "+ /yes/**", "--filter", "- *.no"]
    ...
    test.write_pre(
        "A/MoveEditOnBoth_Bnewer.txt", "Will move and edit on both sides" + randstr(10)
    )
    ...
    test.write_post(
        "B/MovedEditedOnBoth_Bnewer.txt", "BB", mode="at", add_dt=50
    )  # larger too
    ...
    assert (
        test.globread(os.path.join(test.wdB, "backups/*_B/EditOnA.txt")) == "Edit on A"
    ), "not backed up"
    assert (
        test.globread(os.path.join(test.wdA, "backups/*_A/EditOnB.txt")) == "Edit on B"
    ), "not backed up"
    ...
    assert exists(
        os.path.join(test.wdA, "backups/*_A/delB.txt")
    ), "did not backup (delB)"
    assert exists(
        os.path.join(test.wdB, "backups/*_B/delA.txt")
    ), "did not backup (delA)"

    assert exists("A/delB modA.txt")  # Should not have been deleted
    assert (
        "DELETE CONFLICT: File 'delB modA.txt' deleted on B but modified on A. Transfering"
        in stdout
    )
    assert exists("A/delA modB.txt")
    assert (
        "DELETE CONFLICT: File 'delA modB.txt' deleted on A but modified on B. Transfering"
        in stdout
    )
    ...
    assert exists(
        os.path.join(test.wdA, "backups/*_A/sub d‡r/unic°de and space$.txt")
    ), "did not back up unicode"
...
def test_avoid_relist():
    ...
    with open("testdirs/main/relists.json") as fin:
        l = json.load(fin)
        A, B, rA, rB = [
            DictTable(l[k], fixed_attributes=["Path", "Size", "mtime"])
            for k in ["A", "B", "rA", "rB"]
        ]
    ...
    rfA = {
        (f["Path"], f["Size"]) for f in rA if not f["Path"].startswith(".syncrclone")
    }
    rfB = {
        (f["Path"], f["Size"]) for f in rB if not f["Path"].startswith(".syncrclone")
    }
    assert fA == rfA
    assert fB == rfB
...
def test_move_attribs(attrib):
    ...
    for c in moved:
        assert (
            f"Move on B: 'file{c}.txt' --> 'file{c}_moved.txt'" in stdout
        ), f"{attrib} file{c} didn't move"
    for c in too_many:
        assert (
            f"Too many possible previous files for 'file{c}_moved.txt' on A" in stdout
        ), f"{attrib} file{c} failed multiple"
    for c in notmoved:
        assert (
            f"Move on B: 'file{c}.txt' --> 'file{c}_moved.txt'" not in stdout
        ), f"{attrib} file{c} moved"
...
def test_no_hashes():
    ...
    assert (
        "WARNING No common hashes found and/or one or both remotes do not provide hashes. Falling back to 'mtime'"
        in stdout
    )
...
@pytest.mark.parametrize(
    "conflict_mode,tag_conflict",
    itertools.product(
        ("A", "B", "older", "newer", "smaller", "larger", "tag"), (True, False)
    ),
)
...
@pytest.mark.parametrize(
    "backup,sync", itertools.product((True, False, None), (True, False))
)
...
def test_dry_run():
    ...
    shutil.move("A/movemod", "A/movedmod")
    test.write_post("A/movedmod", "ABC")
    os.remove("A/del")
...
def test_logs():
    ...
    assert {os.path.basename(l) for l in logsA} == {
        os.path.basename(l) for l in logsB
...
def test_three_way():
    ...
    assert {
        ("missing_inA", "fileC.txt"),
        ("missing_inB", "file1.txt"),
        ("missing_inB", "file2.txt"),
        ("missing_inB", "file3.txt"),
    } == test.compare_tree(A="A", B="C")

    test.sync()
    assert test.compare_tree(A="A", B="C") == set()
    assert test.compare_tree(A="A", B="B") == {
        ("missing_inB", "fileC.txt")
    }  # Should still miss that
...
def test_local_mode():
    ...
    with open("logs/bla.txt", "wt") as f:
        f.write("bla")
...
def test_and_demo_exclude_if_present():
    ...
    diffs = test.compare_tree()
    assert {
        ("missing_inA", "sub2/ignore"),  # The ignore file isn't transfered
...
def test_version_warning(version):
    ...
    if version and version.startswith("20200825.0"):
        assert (
            "WARNING Previous behavior of conflict_mode changed. Please update your config"
            in stdout
        )
    else:
        assert (
...
@pytest.mark.parametrize(
    "emptyA,emptyB,avoid_relist",
    itertools.chain(
        itertools.product([True, False, None], [True, False, None], [False]),
...
@pytest.mark.parametrize(
    "always,compare,renamesA,renamesB,conflict_mode",
    itertools.product(
        [True, False],
        ["mtime", "size"],
        ["mtime", None],
...
def test_no_modtime(always, compare, renamesA, renamesB, conflict_mode):
    ...
    with lzma.open("A/.syncrclone/A-mmm_fl.json.xz") as fA, lzma.open(
        "B/.syncrclone/B-mmm_fl.json.xz"
    ) as fB:
    ...
    assert mtimeA == (
        always or compare == "mtime" or renamesA == "mtime" or conflict_mode == "newer"
    )
    assert mtimeB == (
        always or compare == "mtime" or renamesB == "mtime" or conflict_mode == "newer"
    )
...
def test_workdir_overlap():
    ...
    with pytest.raises(syncrclone.cli.ConfigError):
        test_main(
            "aliasA1:", "mtime", "aliasA1:aa", "B", "size", None, "size", debug=True
        )
